...
type Audio = 'audio/ogg' | 'audio/mpeg' | 'audio/mpeg, audio/ogg'
...
export const MediaPickerWithThumbnails: FC<ThumbnailsProps> = ({ onThumbnailChange, ...props }) => {
	...
	useEffect(() => {
		// const thumbnail = await fetch(`https://avatar.tobi.sh/${generateSnowflake()}.png`)
...
export const Tag = ({ as: Component = 'div', children, label, className }: PropsWithChildren<TagProps>) => {
	return (
		...
		>
			{label && <label className={classNames('flex items-center rounded-full h-full px-2')}>{label}</label>}
