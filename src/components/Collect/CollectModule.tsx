...
import { PUBLICATION_REVENUE_QUERY } from '@/components/Crowdfund'
...
import { CollectModuleFields } from '@/graphql/CollectModuleFields'
import {
	CashIcon,
	ClockIcon,
	CollectionIcon,
...
import { CONNECT_WALLET, ERROR_MESSAGE, ERRORS, LENSHUB_PROXY, POLYGONSCAN_URL, RELAY_ON } from 'src/constants'
...
const CollectModule: FC<Props> = ({ count, setCount, post }) => {
	...
	const { data: revenueData, loading: revenueLoading } = useQuery(PUBLICATION_REVENUE_QUERY, {
		variables: {
			request: {
				publicationId: post?.__typename === 'Mirror' ? post?.mirrorOf?.id : post?.pubId ?? post?.id,
			},
		},
		skip: !post?.id,
	})
			...
			<div className="p-5">
				{collectModule?.followerOnly && (
					<div className="pb-5">
						<ReferenceAlert
							...
							isSuperFollow={post?.profile?.followModule?.__typename === 'FeeFollowModuleSettings'}
							action="collect"
						/>
					</div>
				)}
				<div className="pb-2 space-y-1.5">
					<div className="flex items-center space-x-2">
						{post?.__typename === 'Mirror' && <SwitchHorizontalIcon className="w-5 h-5 text-brand" />}
						{post?.metadata?.name && <div className="text-xl font-bold">{post?.metadata?.name}</div>}
					</div>
					...
					<ReferralAlert mirror={post} referralFee={collectModule?.referralFee} />
				</div>
				...
				<div className="space-y-1.5">
					<div className="block space-y-1 sm:flex sm:space-x-5 item-center">
						<div className="flex items-center space-x-2">
							...
							>
								<Collectors
									pubId={post?.__typename === 'Mirror' ? post?.mirrorOf?.id : post?.pubId ?? post?.id}
								/>
							</Modal>
						</div>
