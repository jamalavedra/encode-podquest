...
export const useZodForm = <T extends ZodSchema<FieldValues>>({ schema, ...formConfig }: UseZodFormProps<T>) => {
	return useForm({
		...formConfig,
...
export const FieldError: FC<FieldErrorProps> = ({ name }) => {
	...
	return <div className="mt-1 text-sm font-bold text-red-500">{'error.message'}</div>
}

interface Props<T extends FieldValues = Record<string, unknown>> extends Omit<ComponentProps<'form'>, 'onSubmit'> {
