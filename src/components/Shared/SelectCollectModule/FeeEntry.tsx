...
import { EnabledModule, Erc20 } from '@/types/lens'
...
const feeDataSchema = object({
	collectLimit: string().min(1, { message: 'Invalid value' }).max(20, { message: 'Invalid value' }).nullable(),
	value: string().min(1, { message: 'Invalid value' }).max(20, { message: 'Invalid value' }),
	referralFee: string().min(1, { message: 'Invalid Referral fee' }).max(20, { message: 'Invalid Referral fee' }),
})
		...
		<div className="space-y-5">
			...
			<Form form={form} className="space-y-4" onSubmit={({}) => {}}>
				<div>
					...
					<select
						className="w-full bg-white rounded border border-gray-300 outline-none dark:bg-gray-800 disabled:bg-gray-500 disabled:bg-opacity-20 disabled:opacity-60 dark:border-gray-700/80 focus:border-brand-500 focus:ring-brand-400"
				...
				<Input
					...
					helper={
						<span>
							When someone mirrors the publication they will get some reward in percentage for referring
							it.
						</span>
					}
					type="number"
				...
				<button
					className="p-2 bg-gray-700 flex justify-center rounded cursor-pointer hover:bg-gray-800 text-white"
					disabled={
						!form.watch('value') ||
