...
export const getVideo = (media: MediaSet[]): Media | null => {
	const video = media.find(({ original: { mimeType } }) => mimeType.startsWith('audio'))?.original
...
export const normalizeUrl = (url: string, mimeType?: string): string => {
	...
	if (parsed.host === 'ipfs.infura.io') parsed.host = 'podquest.infura-ipfs.io'
	if (parsed.protocol == 'ipfs:') {
		return `https://podquest.infura-ipfs.io/ipfs/${
